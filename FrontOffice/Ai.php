...
<div id="fo-ai-widget" class="fo-ai-widget" aria-expanded="false" data-page="<?php echo htmlspecialchars($aiCurrentPage, ENT_QUOTES, 'UTF-8'); ?>">
    <button type="button" class="fo-ai-widget__trigger" aria-label="Ouvrir l'assistant">
        ...
        <span class="fo-ai-widget__label">Demandez-moi</span>
    </button>
...
<script>
    (function () {
        ...
        function detectLanguage(questionRaw) {
            ...
            var enSignals = ['where', 'order', 'track', 'delivery', 'arrive', 'cancel', 'payment', 'secure', 'what', 'about', 'thanks', 'thank', 'bye', 'goodbye'];
            var frSignals = ['ou', 'commande', 'suivi', 'livraison', 'arrivera', 'annuler', 'paiement', 'securise', 'merci', 'au revoir', 'salut'];
            var enScore = 0;
            var frScore = 0;
            enSignals.forEach(function (w) {
                if (new RegExp('\\b' + w + '\\b').test(q)) enScore++;
            });
            frSignals.forEach(function (w) {
                if (new RegExp('\\b' + w + '\\b').test(q)) frScore++;
            });
        ...
        function getLocalFallbackReply(questionRaw) {
            ...
            if (
                ...
                if (currentPage !== 'commande.php') {
                    return lang === 'en'
                        ? { answer: 'Here is your order tracking link:', link: { url: 'track.php', label: 'Track my order' } }
                        : { answer: 'Voici où se trouve votre commande :', link: { url: 'track.php', label: 'Suivre ma commande' } };
                }
                return lang === 'en'
                    ? { answer: 'Please complete your order first, then use this tracking link:', link: { url: 'track.php', label: 'Track my order' } }
                    : { answer: 'Finalisez d\'abord votre commande, puis utilisez ce lien :', link: { url: 'track.php', label: 'Suivre ma commande' } };
            }

            if (/(\bthanks\b|thank you|\bthx\b|merci)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'You are welcome. If you want, I can also help with products, recipes, or order tracking.'
                        : 'Avec plaisir. Je peux aussi vous aider pour les produits, les recettes ou le suivi de commande.'
                };
            }
            if (/(\bbye\b|goodbye|see you|au revoir|a bientot)/.test(question)) {
                return {
                    answer: lang === 'en'
            ...
            if (/(\brecipe\b|\brecipes\b|recette|recettes|plat|meal)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'You can explore the recipes section, open details, and filter to find what fits your needs.'
                        : 'Vous pouvez explorer la section recettes, ouvrir les details et filtrer selon vos besoins.'
                };
            }
            if (/(\bproduct\b|\bproducts\b|produit|produits|categorie|category|categories)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'You can browse products by category, compare prices or promotions, and open product details.'
                        : 'Vous pouvez parcourir les produits par categorie, comparer les prix ou promotions et voir les details produit.'
                };
            }
            if (/(\bfrigo\b|fridge|refrigerator)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'You can add items to Frigo and review them from the Frigo page.'
                        : 'Vous pouvez ajouter des elements au Frigo puis les consulter depuis la page Frigo.'
                };
            }
            if (/(\ballerg|allergene|allergen|calorie|calories|sante|health)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'Check product and recipe details to see allergens, calories, and health-related information.'
                        : 'Consultez les details des produits et recettes pour voir allergenes, calories et infos sante.'
                };
            }
            if (/(\bpromo|promotion|discount|budget|moins cher|cheap)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'You can search promoted items and compare prices directly in the product list.'
                        : 'Vous pouvez rechercher les promotions et comparer les prix directement dans la liste produits.'
                };
            }
            if (/(\bhello\b|\bhi\b|hey|bonjour|salut)/.test(question)) {
                return {
                    answer: lang === 'en'
                        ? 'Hi! I can help with products, recipes, Frigo, payment, delivery, and order tracking.'
                        : 'Bonjour ! Je peux vous aider pour produits, recettes, frigo, paiement, livraison et suivi.'
                };
            }
            return {
                answer: lang === 'en'
